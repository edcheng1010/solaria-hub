...
- **display:** `pixel`, `image`, `text`, `scroll`, `rgb`, `brightness`, `orientation` (capability MUST include `width`, `height`, and `depth`)
...
| `sound` | `beep`, `play`, `stop`, `set_volume` | Audio output |
...
Additionally, volume control is available if the port advertises the `volume` feature:
```json
...
### 6.5 `sensor.subscribe` Flow-Control
...
### 6.6 `system.subscribe` and Hub-Internal Metrics
...
### 6.7 Feature-to-Command Mapping
...
SSP follows semantic versioning. This document defines v0.6 (draft). Breaking changes increment the major version. Bridges and clients MUST declare their supported SSP version in capability messages and connection handshakes.

---

## 9. Future Considerations (v0.7+)
