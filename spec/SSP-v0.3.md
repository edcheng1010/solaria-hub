---
# Solaria Standard Protocol (SSP) — v0.3
...
- **orientation:** `pitch`, `roll`, `yaw`, `acceleration`, `gesture` (gestures emit discrete events: `shake`, `tap`, `double_tap`, `fall`, `face_up`, `face_down`)
...
| `gesture` | Generates gesture events |
| `beep` | `sound.beep` |
...
SSP follows semantic versioning. This document defines v0.3 (draft). Breaking changes increment the major version. Bridges and clients MUST declare their supported SSP version in capability messages and connection handshakes.

---

## 9. Future Considerations (v0.4+)
